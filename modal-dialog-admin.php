...
class modal_dialog_plugin_admin {
	...
	function on_show_page() {
		...
		if ( isset( $_GET['config'] ) && $config == '' ) {
			$config = intval( $_GET['config'] );

			if ( $config == 0 || $config > $genoptions['numberofmodaldialogs'] ) {
				$config = 1;
			}
		} else {
			$config = 1;
	...
	function general_config_meta_box( $data ) {
		...
		<table>
			<tr>
				...
				<td>
					<input type="text" id="numberofmodaldialogs" name="numberofmodaldialogs" size="5" value="<?php echo intval( $genoptions['numberofmodaldialogs'] ); ?>" />
				</td>
			</tr>
			...
			for ( $counter = 1; $counter <= $numberofdialogs; $counter ++ ): ?>
				...
				<option value="<?php echo $counter ?>" <?php if ( $config == $counter ) {
					echo 'SELECTED';
						...
						echo " (" . esc_html( $tempoptions['dialogname'] ) . ")";
					} ?></option>
			<?php endfor; ?>
		</SELECT>
	...
	function dialog_config_meta_box( $data ) {
		...
		$genoptions = $data['genoptions'];
		?>
		<input type='hidden' value='<?php echo intval( $config ); ?>' name='configid' id='configid' />
		<table>
			<tr>
				<td style="width:250px">Dialog Name</td>
				<td>
					<input type="text" id="dialogname" name="dialogname" size="40" value="<?php echo esc_html( $options['dialogname'] ); ?>" />
				</td>
			</tr>
			<tr>
				<td>Activate</td>
			...
			<tr>
				...
				<td><input type="text" id="delay" name="delay" size="5" value="<?php echo esc_html( $options['delay'] ); ?>" /></td>
			</tr>
			<tr>
				<td>Web Site Address</td>
				<td colspan=3>
					<input type="text" id="contenturl" name="contenturl" size="40" value="<?php echo esc_url( $options['contenturl'] ); ?>" />
				</td>
			</tr>
			<tr>
	...
	function dialog_config_visibility_meta_box( $data ) {
		...
		<table>
			...
			<tr>
				...
				<td colspan="3"><input type="text" id="pages" name="pages" style="width: 100%" value="<?php echo esc_html( $options['pages'] ); ?>" />
				</td>
			</tr>
			<tr>
				<td>Pages and posts not to display dialog on (comma-separated IDs)</td>
				<td colspan="3">
					<input type="text" id="excludepages" name="excludepages" style="width: 100%" value="<?php echo esc_html( $options['excludepages'] ); ?>" />
				</td>
			</tr>
			<tr>
				<td>URLs not to display display on (comma-separated URL strings)</td>
				<td colspan="3">
					<input type="text" id="excludeurlstrings" name="excludeurlstrings" style="width: 100%" value="<?php echo esc_html( $options['excludeurlstrings'] ); ?>" />
				</td>
			</tr>
			<tr>
	...
	function dialog_config_cookie_meta_box( $data ) {
		...
		<table>
			<tr>
				...
				<td>
					<input type="text" id="cookieduration" name="cookieduration" size="4" value="<?php echo intval( $options['cookieduration'] ); ?>" />
				</td>
				<td>Number of times to display / before displaying modal dialog</td>
				<td>
					<input type="text" id="numberoftimes" name="numberoftimes" size="4" value="<?php echo intval( $options['numberoftimes'] ); ?>" />
				</td>
			</tr>
			<tr>
				<td>Counter Mode</td>
				...
				<td>
					<input type="text" id="displayfrequency" name="displayfrequency" size="4" value="<?php echo( $options['displayfrequency'] == '' ? '1' : intval( $options['displayfrequency'] ) ); ?>" />
				</td>
				</td>
			</tr>
			<tr>
				<td>Cookie Name</td>
				<td>
					<input type="text" id="cookiename" name="cookiename" size="30" value="<?php echo esc_html( $options['cookiename'] ); ?>" />
				</td>
			...
			<tr>
				...
				<td>
					<input type="text" id="sessioncookiename" name="sessioncookiename" size="30" value="<?php if ( $options['sessioncookiename'] != '' ) {
						echo esc_html( $options['sessioncookiename'] );
					} else {
	...
	function dialog_config_faq_meta_box() {
		...
		<h2>What are the cookies settings for?</h2>

		<p>Cookies are related to knowing if a visitor has seen a pop-up dialog to avoid displaying to them every single time they visit a certain page on your site. You can have a pop-up stop appearing after a visitor has seen it X times, or have it start appearing after they have visited a page X times. You can specify how long the cookies are good for. After that period, it will reset these X times. For the period to display, it can be used to only display to the visitor every X visits to the page. The show once per session setting means that the pop-up won't appear more than X times during a session (all visits to your site before they close their browser).</p>

		<p>For the cookiname name and cookie session name, they are used to determine the names of the cookies. If they are the same across all of your pop-ups, they the other rules will apply to all pages. For example, if you say to display X times, with X set to 10, and all cookies have the same name, then all pop-ups shown on any page will count towards that maximum display times of 10.</p>

		<p>Finally, the ability to set the cookie manually is a more advanced feature to allow developers to decide how and when the cookie should be set.</p>
